#!/bin/bash
# Create DMG with custom background for SoundBridgePlus
...
echo -e "${BLUE}----------------------------------------${NC}"
echo -e "${BLUE}  Creating SoundBridgePlus DMG${NC}"
echo -e "${BLUE}----------------------------------------${NC}"

# Check if app bundle exists
if [ ! -d "$APP_BUNDLE" ]; then
    echo "ERROR: SoundBridgePlus.app not found at $APP_BUNDLE"
...
if ! command -v create-dmg &> /dev/null; then
    ...
    cp -R "$APP_BUNDLE" "$PROJECT_ROOT/dist/dmg_temp/SoundBridgePlus.app"
    ln -s /Applications "$PROJECT_ROOT/dist/dmg_temp/Applications"
    rm -f "$DMG_PATH"
    hdiutil create -volname "SoundBridgePlus" -srcfolder "$PROJECT_ROOT/dist/dmg_temp" -ov -format UDZO -fs HFS+ "$DMG_PATH"
    rm -rf "$PROJECT_ROOT/dist/dmg_temp"
...
create-dmg \
    --volname "SoundBridgePlus" \
    $BACKGROUND_ARG \
    --window-pos 200 120 \
    --window-size 660 450 \
    --icon-size 100 \
    --icon "SoundBridgePlus.app" 210 220 \
    --app-drop-link 455 220 \
    --hide-extension "SoundBridgePlus.app" \
    --no-internet-enable \
    "$DMG_PATH" \
    "$APP_BUNDLE"
